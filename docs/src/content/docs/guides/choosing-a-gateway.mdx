...
Token Wallet ships with three built-in gateway adapters: Midtrans Snap, Xendit Checkout, and Stripe Checkout. This guide helps you choose the right one for your app.
...
Token Wallet is gateway-agnostic — you can use multiple gateways in the same app:
...
```ts
import type { PaymentGatewayAdapter } from 'token-wallet';
