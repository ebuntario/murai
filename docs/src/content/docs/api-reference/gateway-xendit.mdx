...
description: "API reference for @token-wallet/gateway-xendit — Xendit Checkout adapter"
---
...
```ts
import { createXenditGateway } from 'token-wallet';
...
| Xendit status | Token Wallet status |
|---|---|
| `PAID` | `success` |
| `SETTLED` | `success` |
| `EXPIRED` | `expired` |
