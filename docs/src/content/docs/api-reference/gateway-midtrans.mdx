...
description: "API reference for @token-wallet/gateway-midtrans — Midtrans Snap adapter"
---
...
```ts
import { createMidtransGateway } from 'token-wallet';
...
| Midtrans status | Token Wallet status |
|---|---|
| `settlement` | `success` |
| `capture` | `success` |
| `cancel` | `failed` |
