...
All domain errors extend the abstract `TokenWalletError` base class, which provides a typed `code` property for programmatic error handling.

## Error hierarchy

```
TokenWalletError (abstract)
...
```ts
import { IdempotencyConflictError } from 'token-wallet';
...
```ts
import { InvalidExpirationError } from 'token-wallet';
...
```ts
import { InvalidMetadataError } from 'token-wallet';
...
```ts
import { GatewayError } from 'token-wallet';
...
## Catching any Token Wallet error

Use the base class to catch all domain errors:

```ts
import { TokenWalletError } from 'token-wallet';

try {
  await wallet.spend(userId, amount, key);
} catch (error) {
  if (error instanceof TokenWalletError) {
