...
description: "API reference for @token-wallet/gateway-stripe -- Stripe Checkout adapter"
---
...
```ts
import { createStripeGateway } from 'token-wallet';
...
Token Wallet validates the `v1` signature and enforces a **5-minute timestamp
...
| Stripe event | Token Wallet status |
|---|---|
...
| Stripe `payment_status` | Token Wallet status |
|---|---|
| `paid` | `success` |
